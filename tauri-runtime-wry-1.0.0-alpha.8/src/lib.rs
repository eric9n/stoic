...
pub type CreateWebviewClosure<T> = Box<
    dyn FnOnce(&EventLoopWindowTarget<Message<T>>, &WebContextStore) -> Result<WindowWrapper>
        + Send,
>;
...
fn create_webview<T: UserEvent, F: Fn(RawWindow) + Send + 'static>(
                ...
                page_load_handler(
                    ...
                    match event {
                        wry::PageLoadEvent::Started => {
                            tauri_runtime::window::PageLoadEvent::Started
                        }
                        wry::PageLoadEvent::Finished => {
                            tauri_runtime::window::PageLoadEvent::Finished
                        }
                    },
                )
            });
    ...
    #[cfg(any(target_os = "windows", target_os = "macos", target_os = "linux"))]
    {
        if let Some(additional_browser_args) = webview_attributes.additional_browser_args {
            ...
            {
                if let Ok(proxy_settings) = ProxySettings::new(&proxy_config) {
                    webview_builder =
